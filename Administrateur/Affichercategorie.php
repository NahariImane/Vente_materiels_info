...
echo '<table border="0" cellspacing="0" class="tab">';
 echo '<caption><h1>Catégories</h1></caption>';
    echo '<tr class="class1">';
    echo '<th>Libelle Categorie</th>';
    echo '<th><a href="Ajoutcategorie.php" class="ajt"><i class="fa fa-plus"></i></a></th>';
    echo '</tr>';
$resultat1 = mysqli_query($connexion ,"SELECT Id_Categorie , Libelle_Categorie FROM categorie");
while($ligne = mysqli_fetch_object($resultat1)) {
echo '<tr align="center"><td class="class2">'. $ligne->Libelle_Categorie.'</td><td class="class3"><button onclick="deleteme('.$ligne->Id_Categorie.')"  name="Delete" class="cc1"><i class="fa fa-trash-o"></i></button><button onclick="updateme('.$ligne->Id_Categorie.')"  name="update" class="cc2"><i class="fa fa-pencil"></i></button></td>
        <script language="javascript">
    function deleteme(delid)
    {
      if(confirm("Vous voulez supprimez cette categorie ??")){
        window.location.href="Supprimercategorie.php?id="+delid+" ";
        return true;
      }
    }   
    function updateme(upid)
    {
      if(confirm("Vous voulez modifier cette categorie ??")){
        window.location.href="Modifiercategorie.php?id="+upid+" ";
        return true;
      }
    }   
    </script></tr>';
