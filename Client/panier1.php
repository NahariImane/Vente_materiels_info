...
<?php 
 include('../Connexion_database.php');
  $requete4=" select  distinct p1.Nom_Produit, p1.Photo,p1.Id_produit, p1.Prix,p1.Quantite,p1.Prix_Promo,count(p1.Id_produit)  as Q from produit p1 , possede p2 where p1.Id_produit=p2.Id_produit and p2.Statut=1 group by p1.Id_produit"  ;
  $P=0;
 ...
 if($P!=0)
 {
   ...
   if($ligne->Prix_Promo!=0)
   {
     ...
     <tr align="center"><td>'. $ligne->Nom_Produit. ' </td><td class="pht"><img src=../'. $ligne->Photo. ' style="width:20%"></td><td>'. $ligne->Prix_Promo. ' </td><td>'. $ligne->Q. ' </td><td><a class="l"  id="myBtn2" href="panier.php?id='.$ligne->Id_produit.'"><i class="fa fa-plus"></i> </a></td><td><button onclick="deleteme('.$ligne->Id_produit.')"  name="Delete"><i class="fa fa-trash-o"></i></button></td>
      ...
      else
      {
           ...
           echo '<div class="tableauPa"> 
  <tr align="center"><td>'. $ligne->Nom_Produit. ' </td><td class="pht"><img src=../'. $ligne->Photo. ' style="width:20%"></td><td>'. $ligne->Prix. ' </td><td>'. $ligne->Q. ' </td><td><a class="l"  id="myBtn2" href="panier.php?id='.$ligne->Id_produit.'"><i class="fa fa-plus"></i> </a></td><td><button onclick="deleteme('.$ligne->Id_produit.')"  name="Delete"><i class="fa fa-trash-o"></i></button></td>
        <script language="javascript">
    function deleteme(delid)
    {
      if(confirm("Vous voulez supprimez ce produit de votre commande??")){
      ...
      else
      {
        ...
        echo '<div class="tableauPa"> 
  <tr align="center"><td>'. $ligne->Nom_Produit. ' </td><td ><img src=../'. $ligne->Photo. ' style="width:20%"></td><td>'. $ligne->Prix_Promo. ' </td><td>'. $ligne->Quantite. ' </td><td><a class="l"  id="Btn" ><i class="fa fa-plus"></i></a>
   </div>';
     echo'
...
<script src="../JS/MODALPanier.js"></script></td><td><button onclick="deleteme('.$ligne->Id_produit.')"  name="Delete"><i class="fa fa-trash-o"></i></button></td>
        <script language="javascript">
    function deleteme(delid)
    {
    ...
    else
    {
      ...
      echo '<div class="tableauPa"> 
  <tr align="center"><td>'. $ligne->Nom_Produit. ' </td><td ><img src=../'. $ligne->Photo. ' style="width:20%"></td><td>'. $ligne->Prix. ' </td><td>'. $ligne->Quantite. ' </td><td><a class="l"  id="Btn" ><i class="fa fa-plus"></i></a>
   </div>';
     echo'
<div id="Modal" class="modal">
  <!-- Modal content -->
...
<script src="../JS/MODALPanier.js"></script></td><td><button onclick="deleteme('.$ligne->Id_produit.')"  name="Delete"><i class="fa fa-trash-o"></i></button></td>
 ...
 echo'<label For="To" style="margin:10px 0 0 1135px;">Total : </label><input type="text"  value="'.$total.'" readonly  id="To">';

  echo'<form method="POST" ><button name="V"style="margin:10px 0 0 1290px;">valider</button></form>';

  echo '<a href="AccueilClient.php" style="color:black;"> <--continuer mes achats</a>';
}
else
